...
import { type FormSchemaUpdate2, formSchemaUpdate2 } from "./schema";
...
const Step2: React.FC<Step2Props> = ({ wedding }) => {
	...
	const form = useForm<FormSchemaUpdate2>({
		resolver: zodResolver(formSchemaUpdate2),
		defaultValues: {
	...
	async function onSubmit(data: FormSchemaUpdate2) {
		const formData = jsonToFormData(data);

		await fetchData(`${AppConfig.BackendUrl}/weddings/${wedding.id}/step-2`, {
			method: "PATCH",
			body: formData,
		});
		setWedding({ step: 3 });
										...
										<FormItem className="flex items-center gap-2">
											<FormControl>
												<Checkbox
													...
													onCheckedChange={(_checked) => {
														if (field.value === true) {
															form.setValue("resepsiWanitaTanggal", undefined);
